    ...
    if sysarg == "hello":
      ...
      async with borg.conversation(bot) as conv:
          try:
              ...
              await event.edit("**Error:** `unblock` @M0BOT `and retry!`")
    elif sysarg == "movies":
      async with borg.conversation(bot) as conv:
          try:
              await conv.send_message("/start")
              ...
              await event.edit("**Error:** `unblock` @MoBOT `and retry!`")
    elif sysarg == "song":
      async with borg.conversation(bot) as conv:
          try:
              await conv.send_message("/start")
